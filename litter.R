...
soc_lit <- ggplot(soc_intact,aes(litter,yi,label=id)) + geom_point() + ylab("SOC") +
  ...
  labs(x=expression(paste(CO[2]," effect on litter production (%)", sep="")),
       y=expression(paste(CO[2]," effect on soil carbon stocks (%)", sep=""))) +
  geom_text_repel() +
...
lit_prod <- ggplot(soc_intact,aes(make_pct(biomass),make_pct(litter),label=id)) + 
  ...
  labs(x=expression(paste(CO[2]," effect on aboveground biomass production (%)", sep="")),
       y=expression(paste(CO[2]," effect on litter production (%)", sep=""))) +
  theme_cowplot(font_size=8)
