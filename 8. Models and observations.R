...
models.p <- ggplot(data, aes(make_pct(Cplant), make_pct(Csoil))) + 
  geom_point(data=models,aes(colour=model,shape=site), alpha=0.2,size=3) + 
  geom_point(aes(shape=Site),size=3) +
  geom_smooth(method = "lm", se=FALSE, color="black", formula = y ~ x) +
  stat_smooth(geom='line',data=models,aes(make_pct(Cplant), make_pct(Csoil),colour=model, group=model), alpha=0.2,method = "lm", se=FALSE) +
