...
abs_robin <- projectRaster(abs,crs="+proj=robin",over=T,method="ngb")
...
show_col(mycols)
mycols <- c(colorRampPalette(rev(brewer.pal(9, 'YlOrRd')))(5)[1:2], 
          colorRampPalette(brewer.pal(9, 'YlGnBu'))(9)) # 
p <- ggplot()+ 
...
difcol <- cpt(pal = "ncl_BlueDarkRed18", n=24)
#show_col(difcol)
...
p.dif.abs <- ggplot()+ 
  ...
  scale_fill_discrete_gradient(
    ...
    limits = c(-12, 12),
    guide = guide_colourbar(nbin = 100, raster = FALSE, frame.colour = "black", ticks.colour = NA)) +
  xlab(expression(paste("(Expected - Upscaled) change in soil C (Mg ", ha^-1,")",sep=""))) +
  coord_equal() + 
  theme_classic(base_size = 8) + 
  theme_opts
...
difcol.rel <- cpt(pal = "ncl_BlueDarkRed18", n=36)
p.dif.rel <- ggplot()+ 
  geom_polypath(data=oceanmap_df, aes(long,lat,group=group),fill= nord["nord5"], size = 0.1) +
...
uncert.col.rel.cequal<- cpt(pal = "imagej_cequal", rev=T, n=7)
...
box.dif.abs <- ggplot(data=eco.df,aes(ESAagg, dif.abs)) + 
  ...
  ylab(expression(paste("(Expected - Upscaled) change in soil C (Mg ", ha^-1,")",sep=""))) + xlab("") +
  theme_classic(base_size = 8) +
  theme(axis.text.x = element_text(size=8,angle = 35, hjust = 1))
...
boxes <- plot_grid( box.rel + theme(plot.margin = unit(c(0.2,0.1,-1,0), "cm")),
                    box.dif.rel + theme(plot.margin = unit(c(0,0.1,0,0), "cm")) + ylab("Expected - Upscaled (%)"),
